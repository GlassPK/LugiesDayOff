...
FONT_LG = pygame.font.Font("assets/fonts/comic_sans.ttf", 72)
...
def load_splash(file_path):
    img = pygame.image.load(file_path)
    img = pygame.transform.scale(img, (GRID_SIZE * 2, GRID_SIZE *3))
...
# Images
hero_walk1 = load_image("assets/character/lildude/player_11.png")
hero_walk2 = load_image("assets/character/lildude/player_12.png")
hero_jump = load_image("assets/character/lildude/player_13.png")
hero_idle = load_image("assets/character/lildude/player_23.png")
hero_images = {"run": [hero_walk1, hero_walk2],
               "jump": hero_jump,
               "idle": hero_idle}

hero_splash = load_splash("assets/character/lildude/player_23.png")

block_images = {"TL": load_image("assets/tiles/crate_01.png"),
                "TM": load_image("assets/tiles/crate_01.png"),
                "TR": load_image("assets/tiles/crate_01.png"),
                "ER": load_image("assets/tiles/crate_01.png"),
                "EL": load_image("assets/tiles/crate_01.png"),
                "TP": load_image("assets/tiles/crate_01.png"),
                "CN": load_image("assets/tiles/crate_01.png"),
                "LF": load_image("assets/tiles/crate_01.png"),
                "SP": load_image("assets/tiles/switch_block.png")}

switch_block_img = load_image("assets/tiles/switch_block.png")
switch_img = load_image("assets/tiles/switch.png")

coin_img = load_image("assets/items/coin.png")
heart_img = load_image("assets/items/heart.png")
oneup_img = load_image("assets/items/green.png")
speed_img = load_image("assets/items/blue.png")
flag_img = load_image("assets/nepal.png")
flagpole_img = load_image("assets/items/flagpole.png")

monster_img1 = load_image("assets/enemies/red1.png")
monster_img2 = load_image("assets/enemies/red2.png")
monster_images = [monster_img1, monster_img2]

flyer_img1 = load_image("assets/enemies/flyer1.png")

flyer_images = [flyer_img1]

icon_img = load_image("assets/icon.png")

bear_img = load_image("assets/gorsey.png")
bear_images = [bear_img]

music_img = load_image("assets/sounds/speaker.png")
muted_img = load_image("assets/sounds/mute.png")

soundon_img = load_image("assets/sounds/soundon.png")
soundoff_img = load_image("assets/sounds/soundoff.png")

snail_img = load_image("assets/items/snail.png")
...
VICTORY_SOUND = pygame.mixer.Sound("assets/sounds/the_best.wav")
...
class Character(Entity):
    ...
    def check_flag(self, level):
        ...
        if len(hit_list) > 0:
            if math.ceil((.5 * ((self.rect.y - HEIGHT) * -1)) // 1.87) == 99:
                self.score+= 100
            else:
                self.score += math.floor((.5 * ((self.rect.y - HEIGHT) * -1)) // 1.87)
...
class Level():
    ...
    def __init__(self, file_path):
        ...
        '''for item in map_data['switch_blocks']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.switch_blocks.append(Switch_block(x, y, img))'''
        ...
        for item in map_data['flyers']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_enemies.append(Flyer(x, y, flyer_images))
        ...
        for item in map_data['speed']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_powerups.append(Speed(x, y, speed_img))

        for item in map_data['snail']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_powerups.append(Snail(x, y, snail_img))
        ...
        self.switch_layer = pygame.Surface([self.width, self.height], pygame.SRCALPHA, 32)
...
class Game():
    ...
    def display_splash(self, surface):
        ...
        surface.blit(hero_splash, (WIDTH / 2 - GRID_SIZE, HEIGHT / 2))
    ...
    def display_stats(self, surface):
        ...
        coins_text = FONT_SM.render("        X  " + str(self.hero.coin_amount), 1, WHITE)
        ...
        if not paused:
            surface.blit(music_img, (WIDTH - 64, HEIGHT - 64))
        else:
            surface.blit(muted_img, (WIDTH - 64, HEIGHT - 64))

        if sound_on:
            surface.blit(soundon_img, (WIDTH - 128, HEIGHT - 64))
        else:
            surface.blit(soundoff_img, (WIDTH - 128, HEIGHT - 64))
        surface.blit(score_text, (WIDTH - score_text.get_width() - 32, 32))
        surface.blit(hearts_text, (32, 32))
        surface.blit(lives_text, (32, 80))
        surface.blit(coins_text, (32, 144))
        surface.blit(icon_img, (32, 64))
        surface.blit(coin_img, (32, 128))
        surface.blit(level_text, (WIDTH / 2 - level_text.get_width() / 2, 32))
    ...
    def draw(self):
        ...
        if self.stage == Game.SPLASH:
            ...
            self.display_message(self.window, "Ready?!!!", "Press 'START' again to resume.")
        elif self.stage == Game.LEVEL_COMPLETED:
            self.display_message(self.window, "Level Complete", "Press any key to continue.")
        elif self.stage == Game.VICTORY:
            self.display_message(self.window, "You Win!", "Press 'START' to restart.")
